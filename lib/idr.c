 ...
 * each entry.  The XA_FREE_TAG is only cleared when all bits in the bitmap
 * have been set.
 *
 ...
 * as a value entry.  Value entries never have the XA_FREE_TAG cleared
...
next:
	bitmap = xas_find_tagged(&xas, max / IDA_BITMAP_BITS, XA_FREE_TAG);
